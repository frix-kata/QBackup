...
namespace QBackup
{
    ...
    public static class Utils 
    {
        ...
        public static void WriteOriginFile(Dir top_dir, QualifiedPath new_root)
        {
            ...
            System.IO.File.WriteAllText($"{new_root.Path}{top_dir.Name}{Constants.BACKUP_INFO_FILE_EXT}", JsonConvert.SerializeObject(info));
        }
        ...
        public static void Extract(IList<ArchivedDir> dirs, QualifiedPath extract_to_folder, string password, ILog log)
        {
            ...
            {
                CreateDirInNewRoot(dir.Dir, extract_to_folder);
            }
            ...
            try
            {
                using (var fr = System.IO.File.OpenRead(archive.GetAbsolutePath()))
                {
                    using (var zf = new ZipFile(fr))
                    {
                        ...
                        {
                            var entry = zf[i];
                            if (!entry.IsFile) throw new InvalidOperationException($"Expected all archived entries to be a file. {entry.Name} in {archive.GetAbsolutePath()} is not.");
                    ...
                    using (var zf = new ZipFile(fr))
                    {
                        ...
                        for (int i = 0; i < zf.Count; i++)
                        {
                            ...
                            using (var zip_stream = zf.GetInputStream(entry))
                            {
                                ...
                                using (Stream fs_output = System.IO.File.Create(dest))
                                {
                                    StreamUtils.Copy(zip_stream, fs_output, buffer);
                                }
                            }
                        }
                    }
        ...
        public static void Verify(File archive, string pass, ILog log)
        {
            try
            {
                using (var fr = System.IO.File.OpenRead(archive.GetAbsolutePath()))
                {
                    ...
                    {
                        if (pass != null) zf.Password = pass;
                        zf.TestArchive(true, TestStrategy.FindAllErrors, (status, message) =>
                        {
        ...
        public static void CreateDirTree(IList<Dir> dirs, QualifiedPath root)
        {
            foreach (var dir in dirs)
            {
                CreateDirInNewRoot(dir, root);
            }
        }

        public static void DoParallel<T>(string action_name, IList<T> dirs, Action<T> a, ILog log, Func<T, string> identifier, Action<T> setup = null, string setup_msg = null)
        {
            log.StartTimer();
        ...
        public static (Dictionary<int, Dir>, Dictionary<int, File>) Deserialize(string path, Dir.Serialized[] dir_data, File.Serialized[] file_data)
        {
            var dirs = new Dictionary<int, Dir>();
            var files = new Dictionary<int, File>();
            foreach (var s in dir_data)
            {
                dirs[s.Id] = Dir.BeginBuild(s);
            }

            foreach (var s in file_data)
            {
                files[s.Id] = File.BeginBuild(s);
            }

            foreach (var s in dir_data)
            {
                Dir.FinishBuild(dirs[s.Id], s.Parent != -1 ? dirs[s.Parent] : null, s.Dirs.Select(x => dirs[x]).ToList(), s.Files.Select(x => files[x]).ToList());
            }

            foreach (var s in dir_data)
            {
                Dir.FinishBuild2(dirs[s.Id], path);
            }

            foreach (var s in file_data)
            {
                File.FinishBuild(files[s.Id], dirs[s.Parent]);
            }
            return (dirs, files);
        }
