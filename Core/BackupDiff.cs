...
namespace QBackup.Core
{
    ...
    /// <summary>
    /// Represents the diff data between the backup source and the archive, that needs to be synchronized.
    /// </summary>
    public class BackupDiff
    {
        ...
        public static HashSet<ArchivedDir> GetAllDirsToDelete(List<Dir> source_dirs, List<ArchivedDir> dest_dirs)
        {
            ...
            foreach (var archived_dir in dest_dirs)
                if (!dirs_by_rel_path.ContainsKey(archived_dir.Dir.RelativePath))
                    res.Add(archived_dir);
            return res;
        }

        public static (HashSet<Dir> _dirs_to_add, Dictionary<ArchivedDir, Dir> _dirs_to_update) GetDirsToAddAndUpdate(
            List<Dir> source_dirs,
            List<ArchivedDir> dest_dirs)
        {
        ...
        public static HashSet<ArchivedDir> GetTopDirsToDelete(HashSet<ArchivedDir> all_dirs_to_delete)
        {
            ...
            foreach (var dir in all_dirs_to_delete)
                ...
                if (!dir_set.Contains(dir.Dir.Parent))
                    res.Add(dir);
            return res;
        }
        ...
        public static bool IsFileContentEqual(Dir dir, ArchivedDir archive)
        {
            ...
            foreach (var file in dir.Files)
            {
                ...
                foreach (var archived_file in archive.ArchivedFiles)
                {
                    ...
                    {
                        found = true;
                        if (file.Size != archived_file.Size || !Utils.AreEqual(file.LastWriteTime, archived_file.LastWriteTime))
                            return false;
                        break;
                    }
                }
        ...
        public void Commit(string pass, int aes_key_size, ILog log)
        {
            ...
            //add
            Utils.DoParallel(
                "Add", DirsToAdd.ToArray(), dir => Utils.Compress(dir, Destination.LocatedIn, pass, aes_key_size, log), log,
                dir => dir.RelativePath, dir => { Utils.CreateDirInNewRoot(dir, Destination.LocatedIn); });

            //update
            Utils.DoParallel(
                "Update", DirsToUpdate.ToArray(), dir => Utils.Compress(dir.Value, Destination.LocatedIn, pass, aes_key_size, log), log,
                dir => dir.Key.Dir.RelativePath, dir =>
                {
            ...
            Utils.DoSingleThread(
                "Move", DirsToMove.ToArray(), dir =>
                {
                    var dest_dir = Utils.CreateDirInNewRoot(dir.Key, Destination.LocatedIn);
                    if (dir.Value.Archive != null)
                        System.IO.File.Move(dir.Value.Archive.GetAbsolutePath(), $"{dest_dir.Path}{dir.Value.Archive.Name}");
            ...
            Utils.DoSingleThread(
                "Delete", TopDirsToDelete.ToArray(), dir => Directory.Delete(dir.Dir.AbsolutePath, true), log, dir => dir.Dir.RelativePath);
        ...
        public void Compare(ILog log)
        {
            ...
            log.WriteLine(
                $"Comparison complete: dirs to add: {DirsToAdd.Count}, to update {DirsToUpdate.Count}, to delete: {AllDirsToDelete.Count}, to move: {DirsToMove.Count}");
        }
