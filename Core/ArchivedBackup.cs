...
namespace QBackup.Core
{
    ...
    /// <summary>
    /// Object that's supposed to represent directory with archived folders.
    /// </summary>
    public class ArchivedBackup
    {
        ...
        public static ArchivedBackup CreateFromPath(QualifiedPath path, string pass, ILog log)
        {
            ...
            if (!r.IsPasswordCorrect(pass))
                throw new InvalidOperationException(
                    "Supplied password doesn't match the password of at least some of the files in the archive.");
            return r;
        }
        ...
        public static ArchivedBackup Deserialize(Serialized s)
        {
            ...
            return new ArchivedBackup {ArchivedDirs = s.ArchivedDirs.Select(x => new ArchivedDir(x, dict.Item1, dict.Item2)).ToList()};
        }
        ...
        public Serialized Serialize()
        {
            ...
            {
                RootDir = LocatedIn,
                Files = ArchivedDirs.Select(x => x.Archive).Where(x => x != null).Concat(ArchivedDirs.SelectMany(x => x.ArchivedFiles))
                                    .Select(x => x.Serialize()).ToArray(),
