...
namespace SymbolicLinkSupport
{
    public static class SymbolicLink
    {
        ...
        private static bool TryGetData(string path, out SymbolicLinkReparseData data)
        {
            using (SafeFileHandle fileHandle = GetFileHandle(path))
            {
                ...
                try
                {
                    ...
                    if (!success)
                    {
                        ...
                        {
                            data = new SymbolicLinkReparseData();
                            return false;
                        }
                        Marshal.ThrowExceptionForHR(Marshal.GetHRForLastWin32Error());
                    }
...
#if NET35
                    data = (SymbolicLinkReparseData)Marshal.PtrToStructure(
                        outBuffer, typeof(SymbolicLinkReparseData));
#else
                    data = Marshal.PtrToStructure<SymbolicLinkReparseData>(outBuffer);
#endif
                }
                finally
                {
        ...
        private static bool IsLink(SymbolicLinkReparseData data)
        {
            return data.ReparseTag == IO_REPARSE_TAG_SYMLINK || data.ReparseTag == IO_REPARSE_TAG_MOUNT_POINT;
        }

        public static bool IsLink(string path)
        {
            if (!TryGetData(path, out SymbolicLinkReparseData data)) return false;
            return IsLink(data);
        }

        public static string GetTarget(string path)
        {
            if (!TryGetData(path, out SymbolicLinkReparseData data)) return null;
            if (!IsLink(data)) return null;

            string target = Encoding.Unicode.GetString(data.PathBuffer,
                data.PrintNameOffset, data.PrintNameLength);

            if ((data.Flags & symlinkReparsePointFlagRelative) == symlinkReparsePointFlagRelative)
            {
